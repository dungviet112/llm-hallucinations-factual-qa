...
model_repos = {
    ...
    "Qwen2.5-7B-Instruct" : ("Qwen", f".*model.layers.{coll_str}.mlp.up_proj", f".*model.layers.{coll_str}.self_attn.o_proj"),
...
def get_stop_token():
    if "llama" or "Llama" in model_name:
        stop_token = 13
...
def load_data(dataset_name):
    if dataset_name in trex_data_to_question_template.keys():
        ...
        full_dataset = []
        for obs in tqdm(trivia_qa['train'].shuffle(seed=42)):
            aliases = []
...
def get_start_end_layer(model):
    if "llama" or "Llama" or "Qwen" in model_name:
...
def get_embedder(model):
    ...
    elif "llama" or "Llama" or "Qwen" in model_name:
...
def get_ig(prompt, forward_func, tokenizer, embedder, model):
    ...
    next_token, _ = get_next_token(input_ids, model)
    prediction_id = next_token.squeeze()[-1].argmax()
